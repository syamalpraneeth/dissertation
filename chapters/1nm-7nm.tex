...
\begin{figure}[!h] \centering
	\includegraphics[width=0.95\linewidth,trim={0 2cm 0 1.5cm},clip]{NG_1-7nm/9.pdf}
	\mycaption{Cluster-sizes and Associated Length Scales}{\cz CAMG building blocks ranging from 7 nm nanoparticles to 1 nm clusters, have a population range $\sim$300 times that of the 1 nm cluster.} %, offering the opportunity to explore size-effects in \gls{camg}s.}
	\label{f:clussizes}
\end{figure}

\newpage
\section{Size-effects in CAMGs}
Chapters~\ref{c:dev} and ~\ref{c:camg} describe in detail the \gls{camg}s and \gls{ng}s made from 3 nm sized clusters. %The models chosen to simulate these glasses involved the synthesis of 3 nm clusters, and their subsequent deposition (for CAMGs) and mechanical compaction (for NGs). 
...
\begin{figure}[!h]
	...
	\mycaption{NG interfaces vary with cluster size}{(a) The cluster compaction model is illustrated. (b)-(f) A frontal sliced view of the NGs, with the atoms colored by their von Mises shear strain made from clusters of sizes 1-7 nm shows the homogenization of interface regions with reducing cluster size.}
	\label{f:NGintersiz}
\end{figure}
